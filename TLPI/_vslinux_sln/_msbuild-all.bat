...
REM ======== ######## USAGE ######## ========
rem If no param, all .sln in this directory will be built.
rem If passed an foo.sln filename (no path prefix), only that .sln will be built.

set param1=%~1

if defined param1 (
	set sln_spec=%batdir%\%param1%
) else (
	set sln_spec=%batdir%\*.sln
)
