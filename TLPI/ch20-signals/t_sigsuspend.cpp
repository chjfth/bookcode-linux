...
main(int argc, char *argv[])
{
	...
	if (sigprocmask(SIG_BLOCK, &blockMask, &origMask) == -1)        // (2)
	...
	if (sigaction(SIGINT, &sa, NULL) == -1)                         // (3)
		errExit("sigaction");
	...
	for (loopNum = 1; !gotSigquit; loopNum++)                       // (4)
	{
		printf("\n"); // Chj: a blank line before a new loop.
		printf("=== LOOP %d\n", loopNum);
	...
	if (sigprocmask(SIG_SETMASK, &origMask, NULL) == -1)            // (5)
		errExit("sigprocmask - SIG_SETMASK");
