...
endlocal & (
	...
	set se_batdir=%batdir%
	set se_parentdir=%parentdir%

	REM And be aware: After `endlocal(...)`, to refer to the dir of this VSPU-StartEnv.bat, 
	REM you should use %se_batdir% instead of %batdir%, because %batdir% will be restored 
	REM back to our caller's dir. Our caller is some bat from d:\gitw\VSPG\_VSPG\boots .
)
...
call :SetEnvVar vspg_USER_BAT_SEARCH_DIRS="%se_batdir%"
REM -- So that (personal)user can put VSPU-CopyOrClean.bat into TLPI root-dir to customized post-build copy/clean.
